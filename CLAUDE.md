...
- パッケージ本体は `src/scriptvedit/`（35モジュール）。`pip install -e .` で
...
- 未対応ヒントは正常動作なので、エラーや実行時警告を出さない。将来 audit/strict
  モードを追加する場合だけ info/warning 級 lint として報告する。
...
- **勝手に `git push` しない。** push は明示的に指示されたときだけ行う。
  （以前は pre-push フックで公開FTPアップロードと zip 生成を行っていたが、どちらも廃止し
  フック自体を削除した。push は素の git push だけを行う。）

...
def describe(kind=None, name=None):
    ...
    for pname in sorted(_EFFECT_PLUGINS):
        ...
        plugins.append({
            ...
            "bakeable": s.bakeable,
            "summary": s.doc,
            "signature": _manifest_signature(pname, globals().get(pname)),
            "params": params,
            "source": s.source_file,
        })
...
#
# `python -m scriptvedit` / `python scriptvedit.py` では本ファイルが __main__ として
# 実行されるため、プラグインが `from scriptvedit import effect_plugin` した時点で
# scriptvedit が別インスタンスとして二重ロードされ、プラグインレジストリが分裂して
# 「同名で二重登録」となり全プラグインが読み込み失敗していた。
# 自動読込の前に自分自身を正規名で sys.modules に登録し、同一インスタンスを参照させる。
if __name__ == "__main__" and "scriptvedit" not in sys.modules:
    sys.modules["scriptvedit"] = sys.modules["__main__"]

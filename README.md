...
- **`_imported/` の意味**: 共有ライブラリから取り込んだ素材の置き場。コピーが残るのでプロジェクトは**自己完結**する（共有ライブラリの無い環境・別マシンへ clone してもそのままレンダできる）。ファイル名・相対パス構造はそのまま維持する（日本語名もリネームしない）。git 管理からは外してよい（`scriptvedit new` の `.gitignore` は除外している）。

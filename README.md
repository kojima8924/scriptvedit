...
- `narrate(text_content, *, speaker=1, speed=1.0, pitch=0.0, volume=1.0, subtitle=True, subtitle_style=None, x=0.5, y=0.9, size=36, ...)`: 字幕窓は音声実長に一致し、音声と字幕は同じ開始時刻に配置される。x/y/size/color/font/box/... は text() と同じ字幕スタイル引数（既定は下部中央+半透明ボックス）。**VOICEVOX起動 + svtts.py が必要**
...
### ビート同期（beat_sync / svbeat）

音声のビート（拍）を検出し、キーフレームやカット点に同期させる。ビート検出は librosa 非依存の `svbeat.py`（numpy/scipy のみ）。
...
from svbeat import beats_to_keyframes, snap_times
...
- `beat_sync(audio_source, *, min_bpm=60, max_bpm=200)`: 解析結果は 素材FFP+bpm範囲 をキーに JSON キャッシュ。**svbeat.py + numpy/scipy が必要**（未導入時は導入手順付きの日本語エラー）
...
- CLI: `python svbeat.py song.mp3`（BPM+先頭20拍を表示）/ `--json`（全結果をJSON出力）
...
`svtestkit.py` はレンダリング結果を SSIM で視覚検証するテスト用ユーティリティ。

```python
import svtestkit

# assert_frame: 指定時刻のフレームが期待画像と一致(SSIM>=threshold)することを検証
score = svtestkit.assert_frame("out.mp4", at=2.5, expected="expected.png", threshold=0.97)

# assert_frames: 複数時刻を一括検証（全時刻を検証してから失敗をまとめて報告）
svtestkit.assert_frames("out.mp4", [(1.0, "f1.png"), (2.5, "f2.png")], threshold=0.95)

# 低レベルAPI: フレーム抽出 / SSIM / 差分統計
frame = svtestkit.extract_frame("out.mp4", 2.5, accurate=True)     # RGB numpy配列 (H,W,3)
s = svtestkit.ssim("a.png", "b.png")
d = svtestkit.frame_diff("a.png", "b.png", out_png="diff.png")     # mean_abs/max_abs/diff_ratio
```
...
- CLI: `python svtestkit.py compare a.png b.png` / `python svtestkit.py frame video.mp4 2.5 -o out.png`
...
```
cd test
python test_snapshot.py        # スナップショットテスト（87テスト）
python test_snapshot.py --update  # スナップショット更新
python test_errors.py          # エラーケーステスト（190テスト）
...
- Python 3.10+
- ffmpeg（PATHに必要）
- Playwright + Chromium（テンプレート/web Object/slide 使用時）
- numpy + scipy（`beat_sync` / svbeat 使用時。scipy はビート検出に必須）

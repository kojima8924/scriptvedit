...
かつての単一ファイル `scriptvedit.py`（8,524行）は廃止し、`src/scriptvedit/` の37モジュール（約15,900行）に分割済み。
...
```
pytest tests/                             # 全574件（スナップショット/エラーケース/素材解決/雛形生成/堅牢性/フォント解決）
...
### 実装済み（旧ロードマップから達成）
- **モジュール分割・パッケージ化**: 単一ファイル（8,524行）→ `src/scriptvedit/` の37モジュール。`pip install -e .` でどのフォルダからでも使える。

...
- `asset(relpath)`: リポジトリの `assets/` を自動発見して絶対パスを返す。
  リポジトリ外に置く場合は環境変数 `SCRIPTVEDIT_ASSETS` で明示できる
...
```
pytest tests/                             # 全311件（スナップショット90 + エラーケース221）
...
```
python tests/test_snapshot.py             # 90件
python tests/test_snapshot.py --update    # スナップショット再生成
python tests/test_errors.py               # 221件
...
- **プラグイン機構**: `@effect` デコレータで、コア（src/scriptvedit/）を編集せず `plugins/*.py` に新エフェクトを登録。人にもAIにも「機能をその場で足す」コストを下げる。

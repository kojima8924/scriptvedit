...
かつての単一ファイル `scriptvedit.py`（8,524行）は廃止し、`src/scriptvedit/` の35モジュール（約13,700行）に分割済み。
...
│   ├── tts.py           VOICEVOX 音声合成（voice / narrate）
...
│   ├── assets.py        素材パス解決（asset / here / layer）
...
`assets/` の探索順（**利用者プロジェクト優先**）:

1. 環境変数 `SCRIPTVEDIT_ASSETS`（明示指定・最優先）
2. カレントディレクトリから上方向に `assets/` を探索
3. 実行中のレイヤーファイルの位置から上方向に探索
4. パッケージ位置から上方向に探索（editable インストール時のリポジトリ同梱 `assets/`）

想定運用は「自分の動画プロジェクトのフォルダで scriptvedit をライブラリとして使い、そのフォルダ固有の `assets/` を持つ」こと。
そのため 2・3 が 4 より先に来る（逆順にすると利用者の `assets/` が永久に無視される）。探索結果はキャッシュしないため、cwd 変更・レイヤー切替に追随する。
...
### 外部連携（VOICEVOX / scriptvedit.tts）

`voice()` は VOICEVOX（`scriptvedit.tts` 経由）で日本語テキストを音声合成し、実長を `duration` に設定した音声Objectを返す。
...
- `scriptvedit.tts.speakers()` で話者スタイルID一覧を取得できる。合成 wav はキャッシュされる
- `scriptvedit.viz` / `scriptvedit.tts` は標準ライブラリのみで動作（VOICEVOX との通信時を除く）
- CLI: `python -m scriptvedit.tts "こんにちは" --speaker 1 -o out.wav` / `--list-speakers`
...
- `narrate(text_content, *, speaker=1, speed=1.0, pitch=0.0, volume=1.0, subtitle=True, subtitle_style=None, x=0.5, y=0.9, size=36, ...)`: 字幕窓は音声実長に一致し、音声と字幕は同じ開始時刻に配置される。x/y/size/color/font/box/... は text() と同じ字幕スタイル引数（既定は下部中央+半透明ボックス）。**VOICEVOX起動が必要**（`scriptvedit.tts` 経由）
...
- `export_metadata(path=None, *, title=None, description=None, tags=None)`: `title` 省略時は `param("title")`、`path` 省略時は `metadata.json`。拡張子で .json（構造化）/ .txt（概要欄用）を切替。`marker()` で打った章が目次になる
...
`dry_run` はキャッシュ生成物（数式PNG・web webm・checkpoint 等）を作らないため、その寸法を**常に不明扱い**にする。
おかげでキャッシュの有無でコマンドが変わらず（実レンダ後にスナップショットが落ちる罠がない）、代わりに寸法依存の経路（pad による SEGV バリア等）は実レンダテスト（`tests/render_all.py`）でカバーしている。
...
```
pytest tests/                             # 全323件（スナップショット91 + エラーケース232）
...
python tests/test_errors.py               # 232件
python tests/render_all.py                # 実レンダリング（重い）
python tests/render_all.py test01 test75  # 指定のみ
```

ファイル指紋（キャッシュ鍵）は mtime ではなく**内容ハッシュ**なので、新規 clone・別マシン・キャッシュ生成後のいずれでもスナップショットは安定する。
改行変換による指紋ずれは `.gitattributes`（作業ツリーを CRLF に固定、`templates/vendor/**` は無変換）で防いでいる。
...
- VOICEVOX エンジン（`voice` / `narrate` 使用時。`scriptvedit.tts` 経由）

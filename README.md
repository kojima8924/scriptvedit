...
かつての単一ファイル `scriptvedit.py`（8,524行）は廃止し、`src/scriptvedit/` の36モジュール（約15,600行）に分割済み。

```
scriptvedit/
├── src/scriptvedit/     パッケージ本体（36モジュール）
...
窓で切り出す）。`draft=True` は半解像度のドラフトレンダ（checkpoint/morph の
キャッシュ鍵は本番と分離される）。`alpha=True` は対応形式（webm 等）で
透過付き出力にする。
...
```
pytest tests/                             # 全535件（スナップショット/エラーケース/素材解決/雛形生成/堅牢性/フォント解決）
...
### 実装済み（旧ロードマップから達成）
- **モジュール分割・パッケージ化**: 単一ファイル（8,524行）→ `src/scriptvedit/` の36モジュール。`pip install -e .` でどのフォルダからでも使える。
